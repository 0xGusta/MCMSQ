...
export default function ChatApp() {
    ...
    const handleConnect = async (provider, walletInfo) => {
        ...
        showPopup(`${walletInfo.name} not installed.`, 'error');
    };
    ...
    const sendMON = async (recipientAddress, amount) => {
        try {
            ...
            const tx = await contract.enviarMon(recipientAddress, amount, { value: amount });
            await tx.wait();
            hidePopup();
            showPopup('MON sent successfully!', 'success');
            await provider.getBalance(account).then(b => setBalance(ethers.formatEther(b)));
            await loadMessages(contract);
        } catch (error) {
            hidePopup();
            const friendlyMessage = getFriendlyErrorMessage(error);
        ...
        <div className="app-container">
                                        ...
                                        <div className="message-bubble system">
                                            ...
                                            <div className="flex items-center gap-2 mt-2 text-xs justify-center">
                                                <span 
                                                    ...
                                                    title={`${new Date(systemMessage.timestamp * 1000).toLocaleDateString()} ${new Date(systemMessage.timestamp * 1000).toLocaleTimeString()}`}
                                                >
                                                    {new Date(systemMessage.timestamp * 1000).toLocaleTimeString()}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div key={group.key} id={`message-group-${group.key}`} className={`message-container ${group.isOwn ? 'own' : 'other'} mt-3`}>
                                    {!group.isOwn && (
                                        <div className="profile-pic-container relative">
                                            ...
                                            >
                                                ...
                                                {group.isOnline && <div className="online-status-dot">online</div>}
                                                {/* ---------------------------------- */}
                                            </button>
                                        </div>
                                    )}
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <div className="msg-btnact flex items-center gap-2 mt-2 text-xs">
                                                            <span 
                                                                className="text-gray-400" 
                                                                title={`${new Date(message.timestamp * 1000).toLocaleDateString()} ${new Date(message.timestamp * 1000).toLocaleTimeString()}`}
                                                            >
                                    ...
                                    {group.isOwn && (
                                        <div className="profile-pic-container">
                                            <button onClick={() => handleProfileClick(account)} className="cursor-pointer">
                ...
                {isWrongNetwork && isConnected ? (
                    ...
                    <div className="text-center">
                        <p className="text-gray-300 mb-4">
                            ...
                            You are in read-only mode. Connect your wallet to participate in the chat.
                        </p>
                        <button
                            onClick={() => setShowWalletConnectModal(true)}
                            className="btn btn-primary"
                        ...
                        >
                            ...
                            {isInitialLoad ? 'Loading...' : 'Connect Wallet'}
                        </button>
                    </div>
                )}
            </footer>
            ...
            {showScrollButton && (
                <button 
                    ...
                    className={`scroll-to-bottom fixed right-10 bottom-36 z-49 ${unseenMessages > 0 ? 'new-message animate-pulse' : ''}`}
                >
            ...
            <ProfileModal
                ...
                isOwnProfile={selectedUserAddress.toLowerCase() === account.toLowerCase()}
                isOwner={isOwner}
