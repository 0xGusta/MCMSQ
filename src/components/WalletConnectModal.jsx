            ...
            <div className="modal-content" onClick={e => e.stopPropagation()}>
                    ...
                    {SUPPORTED_WALLETS.map((wallet) => {
                            ...
                            return (
                                <div key={wallet.rdns} className="btn btn-secondary w-full justify-start text-lg p-4 opacity-50 cursor-not-allowed">
                                    ...
                                    <div className="flex flex-col items-start leading-tight">
                                        ...
                                        <span className="text-xs text-gray-400 font-normal">Only desktop</span>
                                    </div>
                                </div>
                            );
                        }
                        ...
                        return (
                            <button
                                ...
                                title={!isMobile() && !installedWallet ? `${wallet.name} not detected` : `Connect with ${wallet.name}`}
                            >
                                <img src={wallet.icon} alt={wallet.name} className="w-8 h-8 mr-4" />
                                {wallet.name}
                ...
                {isMobile() && (
                    <p className="text-center text-xs mt-4 text-yellow-300">
                        PLEASE COPY THE LINK OF THE SITE AND OPEN IT IN THE METAMASK BROWSER. UNFORTUNATELY, IT IS NOT POSSIBLE TO CONNECT DIRECTLY THROUGH THE BROWSER ON MOBILE.
                    </p>
                )}
            </div>
        </div>
